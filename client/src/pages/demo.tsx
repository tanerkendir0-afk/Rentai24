...
export default function Demo({ isWorkspace = false }: { isWorkspace?: boolean }) {
  ...
  return (
    <div className="fixed inset-0 pt-16 flex bg-background">
                ...
                <div className="mt-3 pt-3 border-t border-border/30">
                  ...
                  <div className="px-1.5 mb-2">
                    <button
                      ...
                      className="w-full flex items-center justify-center gap-2 px-3 py-2 rounded-lg border border-dashed border-primary/30 bg-primary/5 text-primary hover:bg-primary/10 hover:border-primary/50 transition-all text-xs font-medium"
                      data-testid="button-sidebar-new-chat"
                    >
                      <MessageSquarePlus className="w-3.5 h-3.5" />
                      <span>{t("demoPage.newChat")}</span>
                    </button>
                  </div>
              ...
              <div className="p-3 border-t border-border/50">
                <div className="flex items-center justify-between text-[11px] mb-2">
                  <span className="flex items-center gap-1.5 text-muted-foreground font-medium">
                    ...
                    {t("demoPage.tokenUsage")}
                  </span>
                  <span className={`font-semibold tabular-nums ${
                    spendingData.limitReached ? "text-red-400" :
                    spendingData.remaining < 1 ? "text-yellow-400" :
                    "text-foreground"
                  }`}>
                    ${spendingData.spent.toFixed(2)} <span className="text-muted-foreground/50 font-normal">/ ${spendingData.limit.toFixed(2)}</span>
                  </span>
                </div>
      ...
      >
       <div className={`flex-1 flex flex-col min-w-0 h-full ${hasBoost ? "pb-[5.5rem]" : ""}`}>
