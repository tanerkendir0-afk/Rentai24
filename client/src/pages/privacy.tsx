            ...
            <Card className="p-8 sm:p-10 bg-card border-border/50 prose prose-invert max-w-none">
              <div className="space-y-8 text-muted-foreground leading-relaxed">
                {sections.map((section) => {
                  ...
                  const content = t(`privacy.sections.${section}.content`, { defaultValue: "" });
                  const intro = t(`privacy.sections.${section}.intro`, { defaultValue: "" });
                  const rawItems = t(`privacy.sections.${section}.items`, { returnObjects: true, defaultValue: [] });
                  const items = Array.isArray(rawItems) ? (rawItems as string[]) : [];
                  const outro = t(`privacy.sections.${section}.outro`, { defaultValue: "" });
                  ...
                  return (
                    <div key={section}>
                      ...
                      {outro && <p className="mt-3">{outro}</p>}
                      {section === "contact" && (
                        <p>
                          {content}{" "}
