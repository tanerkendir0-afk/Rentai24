...
*   **Admin Panel:** A password-protected interface for administrators to manage RAG (Retrieval Augmented Generation) documents, fine-tune AI models, export training data (JSONL for OpenAI fine-tuning), download agent rules documentation, and monitor system activity. Includes Training Data tab with conversation stats, export filters (min turns, tools-only), and agent rules document download.
