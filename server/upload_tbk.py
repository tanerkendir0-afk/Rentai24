"""
TBK (6098 sayılı Türk Borçlar Kanunu) — Pinecone Yükleme
Kullanım: cd ~/workspace/server && python3 upload_tbk.py
"""
import os, json, time
...
for path in ["tbk-chunks.json", "server/tbk-chunks.json", "../tbk-chunks.json"]:
    if os.path.exists(path):
        with open(path, "r", encoding="utf-8") as f:
            data = json.load(f)
        break
else:
    raise FileNotFoundError("tbk-chunks.json bulunamadı!")

chunks = data["chunks"]
total = len(chunks)
BATCH = 25

print(f"📚 TBK: {total} chunk yüklenecek...\n")
